...
def compute_feat_acts(
...
) -> Float[Tensor, "batch seq three feats"]:
    """
    ...
    feature_act_dir = encoder.W_enc[:, :, :, feature_idx]  # (n_layers, d_in, 3, feats)
    feature_bias = encoder.b_enc[:, feature_idx]  # (3, feats)
    ...
    feat_acts_pre = einops.einsum(
        x_cent, feature_act_dir, "batch seq n_layers d_in, n_layers d_in three feats -> batch seq three feats"
    )
    ...
    if corrcoef_neurons is not None:
        corrcoef_neurons.update(
            einops.rearrange(feat_acts, "batch seq three feats -> three feats (batch seq)"),
    ...
    if corrcoef_encoder_B is not None:
        ...
        feat_acts_pre_B = einops.einsum(
            x_cent_B,
            encoder_B.W_enc,
            "batch seq d_in, d_in three d_hidden -> batch seq three d_hidden",
        )
        ...
        corrcoef_encoder_B.update(
            einops.rearrange(feat_acts, "batch seq three feats -> three feats (batch seq)"),
            einops.rearrange(feat_acts_B, "batch seq three d_hidden -> three d_hidden (batch seq)"),
        )
...
def parse_feature_data(
    ...
    all_feat_acts: Float[Tensor, "... three feats"],
    feature_resid_dir_A: Float[Tensor, "three feats d_model"],
    feature_resid_dir_B: Float[Tensor, "three feats d_model"],
    all_resid_post: Float[Tensor, "... d_model"],
    W_U_A: Float[Tensor, "d_model d_vocab"],
    W_U_B: Float[Tensor, "d_model d_vocab"],
    cfg: SaeVisConfig,
    feature_out_dir: Float[Tensor, "three feats d_out"] | None = None,
    corrcoef_neurons: RollingCorrCoef | None = None,
    corrcoef_encoder: RollingCorrCoef | None = None,
    corrcoef_encoder_B: RollingCorrCoef | None = None,
    ...
    if feature_out_dir is not None:
        ...
        ), f"Num features in feature_out_dir ({feature_out_dir.shape[0]}) doesn't match {len(feature_indices)=}"
    ...
    if layout.feature_tables_cfg is not None and feature_out_dir is not None:
        ...
        if layout.feature_tables_cfg.decoder_cosine_sim_table: # TODO: update
            # TODO: we should probably use exact decoder dir rather than resid dir, but should be fine for residual stream crosscoders
            cosine_sims = F.cosine_similarity(feature_resid_dir_A, feature_resid_dir_B, dim=-1) # [three feats]
            if cosine_sims.dim() == 0:
                cosine_sims = cosine_sims.unsqueeze(0)
                
            cosine_sims = cosine_sims.unsqueeze(-1) # [three feats 1]
            feature_tables_data.update(
                decoder_cosine_sim_values=cosine_sims.tolist(), # [three feats 1]
            )
    ...
    if any(
        ...
        for i, (feat, logit_vector_A, logit_vector_B) in enumerate(zip(feature_indices, logits_A, logits_B)):
            ...
            # Three-refactoring: can do this for indices 1 and 2 too
            if layout.act_hist_cfg is not None:
                feat_acts = all_feat_acts[..., 0, i]
                ...
                feature_data_dict[
                    ...
                    tickmode="5 ticks",
                    title=f"SHARED<br>ACTIVATIONS<br>DENSITY= {frac_nonzero:.3%}",
                )
    ...
    # Three-refactoring: improve this
    feature_stats = FeatureStatistics.create(
    #    data=einops.rearrange(all_feat_acts, "b s three feats -> three feats (b s)")
    )
...
def get_sequences_data(
    tokens: Int[Tensor, "batch seq"],
    feat_acts: Float[Tensor, "batch seq three"],
    ...
    import copy
    three_feat_acts = copy.deepcopy(feat_acts)
    for i in [0]:#,0,1]:#range(3):
        feat_acts = three_feat_acts[:, :, i]
        ...
        for group_idx, group_name in enumerate(indices_dict.keys()):
            seq_data = [
                SequenceData(
                    ...
                    feat_acts=[round(f, 10) for f in feat_acts_coloring[i].tolist()],
                    loss_contribution=[0]*len(token_ids[i]),
                    token_logits=feat_logits[token_ids[i]].tolist(),
                    top_token_ids=[[]]*len(token_ids[i]),
                    top_logits=[[]]*len(token_ids[i]),
                    bottom_token_ids=[[]]*len(token_ids[i]),
                    bottom_logits=[[]]*len(token_ids[i]),
                )
                for i in range(
...
def parse_prompt_data(
    ...
    for i, feat in enumerate(feature_idx):
        ...
        sae_vis_data.feature_data_dict[feat].prompt_data = SequenceData(
            ...
            feat_acts=[round(f, 10) for f in feat_acts[:, i].tolist()],
