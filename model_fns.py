...
class CrossCoder(nn.Module):
    def __init__(self, cfg: CrossCoderConfig):
        ...
        self.W_enc = nn.Parameter(
            torch.nn.init.kaiming_uniform_(torch.empty(2, cfg.d_in, 3, cfg.d_hidden))
        )
        self.W_dec = nn.Parameter(
            torch.nn.init.kaiming_uniform_(torch.empty(cfg.d_hidden, 2, cfg.d_in))
        )
        self.b_enc = nn.Parameter(torch.zeros(3, cfg.d_hidden))
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_enc = einops.einsum(
            x_cent,
            self.W_enc,
            "... n_layers d_model, n_layers d_model three d_hidden -> ... three d_hidden",
        )
        acts = F.relu(x_enc + self.b_enc)
        #x_reconstruct = acts @ self.W_dec + self.b_dec
        x_reconstruct_shared = einops.einsum(
            acts[:, 0, :],
            self.W_dec,
            "... d_hidden, d_hidden n_layers d_model -> ... n_layers d_model",
        )
        x_reconstruct_A = einops.einsum(
            acts[:, 1, :],
            self.W_dec[:, 0, :],
            "... d_hidden, d_hidden d_model -> ... d_model",
        )
        x_reconstruct_B = einops.einsum(
            acts[:, 2, :],
            self.W_dec[:, 1, :],
            "... d_hidden, d_hidden d_model -> ... d_model",
        )
        x_reconstruct_exclusive = torch.stack((x_reconstruct_A, x_reconstruct_B), dim=-2)
        x_reconstruct = x_reconstruct_shared + x_reconstruct_exclusive
        ...
        l1_loss_shared = (acts * total_decoder_norm[None, :]).sum(-1).mean(0)
        l1_loss_exclusive = (acts[:, 1, :] * decoder_norms[None, :, 0] + acts[:, 2, :] * decoder_norms[None, :, 1]).sum(-1).mean(0)
        l1_loss = l1_loss_shared + 1.5*l1_loss_exclusive
        loss = l2_loss + l1_loss # shouldn't there be a times 2 in the l1_loss here?
